...
const MyAddress = ({ navigation, route }) => {
	const { user, saveDeliveryAddress, deleteUserAddress } = useContext(
		authContext
	);
	...
	const handleDelete = (addressToDelete) => {
		deleteAddressHandler(addressToDelete);
	};

	const deleteAddressHandler = (add) => {
		Alert.alert("Deleting", "Do you want to delete this address?", [
			{
				text: "Delete",
				style: "destructive",
				onPress: () => deleteAddressHandler(add),
			},
